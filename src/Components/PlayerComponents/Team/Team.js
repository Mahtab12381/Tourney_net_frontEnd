...
function Team() {
    ...
    const userName = localStorage.getItem('user-info') ? JSON.parse(localStorage.getItem('user-info')).User_id : null;
   ...
   const [Team_logo, setTeam_logo] = useState("");
    ...
    useEffect(() => {
        async function getResult() {
            ...
            const filteredresult = result.filter(obj => obj.Type === 'Player' && obj.Id !== userID);
    ...
    async function handlecreate(){
        ...
        let item={Team_name,Team_logo,Captain_id,Player2_id,Player3_id,Player4_id,Player5_id};
    ...
    async function handlemyteamClick(){
        ...
        result = await result.json();
        const myteam = result.filter(obj => obj.Captain_id === userID||obj.Player2_id === userID||obj.Player3_id === userID||obj.Player4_id === userID||obj.Player5_id === userID);
        setmyteam(myteam);
        setShow(!show);    
    }
    ...
    return (
        <>
            ...
            <div class='content-wrapper'>
                <div class="t-header">
                    <h1>Team</h1>
                    {!show?<button onClick={handlemyteamClick} class='btn btn-primary'>My Teams</button>:<button onClick={()=>setShow(!show)} class='btn btn-primary'>Create team</button>}
                </div>
                {
                    !show?
                    <div class='container-fluid tournament-container'>
                    <label>Enter Team Name:</label>
                    <input onChange={(e)=>setTeam_name(e.target.value)} type="text" class="form-control"></input>
                    < div class='row'>
                        <div class="col-md-6">
                                    ...
                                    <tbody>
                                        {result ? result.map((item) => (
                                            <tr key={item.Id}>
                                                ...
                                                <td>{item.Email}</td>
                                                <td>{item.UserName}</td>
                                                {selectedplayers.includes(item) && selectedplayers.length<4 ? <button disabled class="btn btn-primary">Added</button>
                                                : selectedplayers.length<4 ? <button class="btn btn-primary" onClick={() => setselectedplayersPlayers([...selectedplayers, item]) }>Add</button> 
                            ...
                            <button onClick={handlecreate} class="btn btn-primary">Create Team</button> 
                        </div>
                        <div class="col-md-6">
